...
<div id="wrapper">
    ...
    <div id="page-wrapper" style="height:100%">
        <div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">有偿配套管理</h3>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- /.row -->
...
<script>
    $(document).ready(function() {
        var editor = new $.fn.dataTable.Editor( {
            ...
            ajax: {
                ...
                data:function(data){
                    ...
                    for(var i in data.data){
                        var matching = data.data[i].matching;
                        matching.id=i;
                        matchings.push(JSON.stringify(matching));
                    }
                    result.matchings = "["+matchings.toString()+"]";
                    return result;
                }
            ...
            fields: [ {
                label: "房间号:",
                name: "matching.roomnum"
            }, {
                label: "面积:",
                name: "matching.area"
            }, {
                label: "单价:",
                name: "matching.unitprice",
            }, {
                label: "期限:",
                name: "matching.term",
                type:"datetime",
        ...
        var table =$('#dataTables-example').DataTable({
            ...
            columns: [
                ...
                { data: "matching.roomnum" },
                {
                    data: "matching.usetype",
                    render:function (data,type,row) {
                        if(row.matching.usetype==0){
                            return '<i class="fa fa-building-o fa-fw"></i>公寓'
                        }else if(row.matching.usetype==1){
                            return '<i class="fa fa-bullseye fa-fw"></i>会议室'
                        }else if(row.matching.usetype==2){
                            return '<i class="fa fa-steam fa-fw"></i>储藏室'
                        }
                    }
                },
                { data: "matching.area" },
                { data: "matching.unitprice" },
                {
                    data: "matching.state",
                    render:function (data,type,row) {
                        if(row.matching.state==0){
                            return '<span class = "label label-warning">未出租</span>'
                        }else if(row.matching.state==1){
            ...
            buttons: [
                { extend: "create", editor: editor ,text: '<i class="fa fa-plus">&nbsp;&nbsp;添加有偿配套</i>'},
                { extend: "edit",   editor: editor ,text: '<i class="fa fa-edit">&nbsp;&nbsp;修改有偿配套</i>'},
                { extend: "remove", editor: editor ,text: '<i class="fa fa-trash-o">&nbsp;&nbsp;删除有偿配套</i>'},
        ...
        editor.on("preSubmit",function(e,o,action){
            if(action !== 'remove'){
                var roomnum = this.field('matching.roomnum');
